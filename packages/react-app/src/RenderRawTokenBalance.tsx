...
export const RenderRawTokenBalance: React.FC<RenderRawTokenBalanceProps> = ({ balance, ticker, decimals, chainName, opts }) => {
  return <span>{balance !== undefined && opts?.hideAmount !== true ? formatFloat(formatUnits(balance, decimals), getDecimalsToRenderForTokenTicker(ticker)) : '?'}{opts?.hideTicker !== true && ` ${ticker}`}{opts?.hideChainSeparator !== true && ' on'}{opts?.hideChain !== true && ` ${chainName}`}</span>;
}
