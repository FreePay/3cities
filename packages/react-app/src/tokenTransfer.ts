...
export type TokenTransferForToken = Narrow<TokenTransfer, 'token', Token>; // TokenTransferForToken is a convenience type for the subset of TokenTransfers where a token (and not a native currency) is being transferred. But where possible, clients should prefer using TokenTransfer to abstract over the type of transfer

export type TokenTransferForNativeCurrency = Narrow<TokenTransfer, 'token', NativeCurrency>; // TokenTransferForNativeCurrency is a convenience type for the subset of TokenTransfers where a native currency (and not a token) is being transferred. But where possible, clients should prefer using TokenTransfer to abstract over the type of transfer
