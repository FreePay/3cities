...
export type CheckoutSettings = Readonly<{
  ...
  hideReceiverAddress?: boolean; // iff true, the payment receiver address will be hidden in the payment UX. WARNING malicious payment requests can be crafted where the receiver address is set to badguy.eth and hidden, but `note` is set to "To: goodguy.eth". Design note: we didn't want to enable hiding the receiver address as it can result in malicious payments; however, some senders/buyers were copying the receiver address and manually sending tokens in their wallet, which breaks embedded merchant integrations where payment must be made using the modal because iframe message passing is utilized to submit payment details to the merchant's server, so we allow hiding the receiver address to mitigate this edge case.
}>
